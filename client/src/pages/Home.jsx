...
const Home = () => {
  ...
  const ethersFunc = async() => {
    try{
      ...
      const contractTest = new ethers.Contract(contractAddress, AbiData, signer)
  ...
  const donate = async() => {
    try{
      const donateMoney = await contract.Donate({value: ethers.utils.parseEther(amount)})
  ...
  const usersDonations = async() => {
    try{
      const DonorRecords = await contract.donations(address, 0)
  ...
  return (
    <div>
      <div>
      <input type="number" name="" value={amount} required onChange={(e) => setAmount(e.target.value)} id="" />
        <button className='btn btn-success' onClick={ethersFunc}>Connect</button>
        <br/>
        <br/>
        <br/>
        <br/>
